...
def log_login_attempts(report_file):
    ...
    try:
        with open(report_file, "a") as f:
            f.write("\nRoot Login Attempts:\n")
            for entry in utmp.UtmpFile(WTMP_FILE):
                if entry.ut_user == 'root':
                    f.write(f"{entry.ut_line} {entry.ut_user} {entry.ut_host} {datetime.fromtimestamp(entry.ut_tv.tv_sec)}\n")
    except Exception as e:
        print("Unexpected error:", e)
